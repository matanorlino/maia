...
class Teacher extends General_DataTables {
	...
	public function upload_ppt($lessonname){
		...
		if (!empty($_FILES)) {
			...
			$fileid = $this->main->getPptId($fileName)[0]->id;
			...
			if ($fileid != null) {
				...
				if ($validateFile == null) {
					$this->main->insertPpt($lessonid, $fileid);
				}
			}else{
	...
	public function upload_pdf($lessonname){
		...
		if (!empty($_FILES)) {
			...
			$fileid = $this->main->getPdfId($fileName)[0]->id;

			if ($fileid != null) {
				$lessonid = $this->main->getLessonIdByName($lessonname)[0]->rowID;
				$validateFile = $this->main->validatePdf($lessonid, $fileid);
				// exit(var_dump($validateFile));
				
				if ($validateFile == null) {
					$this->main->insertPdf($lessonid, $fileid);
				}
			}else{
				$editFilename = str_replace("(", "-", $fileName);
				$editFilename = str_replace(")", "-", $editFilename);
	...
	public function upload_video($lessonname){
		...
		if (!empty($_FILES)) {
			...
			$fileid = $this->main->getVideoId($fileName)[0]->id;

			
			if ($fileid != null) {
				$lessonid = $this->main->getLessonIdByName($lessonname)[0]->rowID;
				$validateFile = $this->main->validateVideo($lessonid, $fileid);

				if ($validateFile == null) {
					$this->main->insertVideo($lessonid, $fileid);
				}
			}else{
				$editFilename = str_replace("(", "-", $fileName);
				$editFilename = str_replace(")", "-", $editFilename);
